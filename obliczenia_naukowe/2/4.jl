...
# making a polynomial out of coefficients
my_poly::Polynomial = Polynomial(reverse(coef))
println("Polynomial: \n")
println(my_poly, "\n")

# using roots() in order to calculate, well, roots
my_poly_roots::Vector{Float64} = roots(my_poly)
#println(my_poly_roots)
#
# wilkinsons polynomial using fromroots
...
# slight change in coefficients in order to reproduce Wilkinson's experiment

wil_coef::Vector{Float64} = [1, -210.0-2.0^(-23.0), 20615.0,-1256850.0,
                        53327946.0,-1672280820.0, 40171771630.0, -756111184500.0,          
                        11310276995381.0, -135585182899530.0,
                        1307535010540395.0,     -10142299865511450.0,
                        63030812099294896.0,     -311333643161390640.0,
...
# using roots() in order to calculate, well, roots
my_poly_roots_b::Vector{Float64} = roots(my_poly_b)
#println(my_poly_roots)
#
# wilkinsons polynomial using fromroots
